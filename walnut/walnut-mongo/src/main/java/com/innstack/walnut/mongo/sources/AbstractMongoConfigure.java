package com.innstack.walnut.mongo.sources;
    ...
    public MongoDbFactory mongoDbFactory() throws Exception {
        if (isAuth) {
            ...
            if (host.contains(",")) {
                ...
                for (int i = 0; i < len; i++) {
                    String uri = String.format("mongodb://%s:%s@%s:%d/%s", username, password, hosts[i], port, database);
                    if (i == 0) {
                        sURI = uri;
                    } else {
                        sURI = sURI + "," + uri;
            ...
            MongoClientOptions mongoClientOptions = MongoClientOptions.builder().socketKeepAlive(socketKeepAlive).build();
